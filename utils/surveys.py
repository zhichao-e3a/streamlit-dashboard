...
def find_col_name(cols, patterns):
    ...
    logger.warning("No matching columns for %s", patterns)
...
def yes_no_mapping(text, default):
    ...
    # * The 其他 field will default to "Yes" here.
...
async def upsert(df_pre, df_post):
    ...
    logger.info(f"UPDATING PRE_SURVEY, POST_SURVEY: {st.session_state.mode}")
    ...
    surgery_history_col = find_col_name(
        cols, [r".*17\.您是否做过剖腹产或子宫相关手术？.*"]
    )
    ...
    last_menstrual_col = find_col_name(
        cols, [r".*8\.您的最后一次月经大概是什么时候？.*"]
    )
    edd_col = find_col_name(cols, [r".*10\.预产期是？.*"])
    last_delivery_date_col = find_col_name(
        cols, [r".*15\.最近一次分娩大概是什么时候？.*"]
    )
    ...
    for record in mapped_data_pre:
        ...
        if not contact_number:
            txt = f"INVALID RECORD NUMBER"
            logger.warning(txt)
            messages.append(txt)
            continue

        if contact_number in pre_contact_set:
            txt = f"[{repeat_count:02d}] REPEATED PRE-SURVEY RESPONSE FOR {contact_number}"
            logger.info(txt)
            messages.append(txt)
            repeat_count += 1
            continue
    ...
    await mongo.upsert_documents_hashed(
        coll_name="PRE_SURVEY", records=pre_unique_records, id_fields=["mobile"]
    )

    txt = f"UPSERTED {len(pre_unique_records)} RECORDS INTO PRE_SURVEY"
    logger.info(txt)
    messages.append(txt)
    ...
    ga_exit_col = find_col_name(
        cols, [r"分娩时.*孕周.*第几周.*几天|孕周.*例如.*38.*周|孕周"]
    )
    ...
    dur_contr_to_birth_col = find_col_name(
        cols, [r"从开始.*宫缩.*到.*出生.*持续.*多长.*时间"]
    )
    dur_room_to_birth_col = find_col_name(cols, [r"从进入.*产房.*到.*出生.*多长.*时间"])
    dur_inform_to_room_col = find_col_name(
        cols, [r"从.*被告知.*进入.*产程.*到.*转入.*产房.*间隔.*多久"]
    )

    # Usage related
    usage_reason_col = find_col_name(cols, [r"使用.*主要.*目的|使用.*目的"])
    awareness_col = find_col_name(
        cols, [r"使用.*期间.*(感受到|察觉).*(宫缩|收缩).*逐渐加剧"]
    )
    influence_col = find_col_name(
        cols, [r"(读数|数据).*(是否|有无).*(去医院|生产).*产生.*影响"]
    )
    ...
    informed_doc_col = find_col_name(
        cols, [r"是否.*告诉.*医生.*使用.*萌动|是否.*告知.*医生"]
    )
    ...
    for record in mapped_data_post:
        ...
        if contact_number in post_contact_set:
            txt = f"[{repeat_count:02d}] REPEATED POST-SURVEY RESPONSE FOR {contact_number}"
            logger.info(txt)
            messages.append(txt)
            repeat_count += 1
            continue
    ...
    await mongo.upsert_documents_hashed(
        coll_name="POST_SURVEY", records=post_unique_records, id_fields=["mobile"]
    )

    txt = f"UPSERTED {len(post_unique_records)} RECORDS INTO POST_SURVEY"
    logger.info(txt)
    messages.append(txt)
